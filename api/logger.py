...
def save_logs(msg):
    try: os.mkdir(config["logging-dir"])
    except: pass
    msg = unicode_filter(msg)
    log_dir = config['logging-dir']
    limit = os.environ.get("LOGGER_MAXLIMIT", False)
    # split limit into (num, unit)
    if limit:
        new_limit = ["", ""]
        for i,x in enumerate(str(limit)):
            try: 
                int(x)
                new_limit[0] += x
            except:
                new_limit[1] = x[i:]
                break
        limit = [int(new_limit[0]), new_limit[1]]
